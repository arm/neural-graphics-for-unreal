...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MyFloatingPawnMovement)

UMyFloatingPawnMovement::UMyFloatingPawnMovement(const FObjectInitializer& ObjectInitializer)
...
bool UMyFloatingPawnMovement::LimitWorldBounds()
{
...
void UMyFloatingPawnMovement::ApplyControlInputToVelocity(float DeltaTime)
{
...
bool UMyFloatingPawnMovement::ResolvePenetrationImpl(const FVector& Adjustment, const FHitResult& Hit, const FQuat& NewRotationQuat)
{
	bPositionCorrected |= Super::ResolvePenetrationImpl(Adjustment, Hit, NewRotationQuat);
	return bPositionCorrected;
}

FRotator UMyFloatingPawnMovement::GetDeltaRotation(float DeltaTime) const
{
	FRotator rotator = FRotator::ZeroRotator;
	rotator.Pitch = GetAxisDeltaRotation(RotationRate.Pitch, DeltaTime);
	rotator.Yaw = GetAxisDeltaRotation(RotationRate.Yaw, DeltaTime);
	rotator.Roll = GetAxisDeltaRotation(RotationRate.Roll, DeltaTime);
	return rotator;
}

FRotator UMyFloatingPawnMovement::ComputeOrientToMovementRotation(const FRotator& CurrentRotation, float DeltaTime, FRotator& DeltaRotation) const
{
	// Rotate toward direction of acceleration.
	return Velocity.GetSafeNormal().Rotation();
}

float UMyFloatingPawnMovement::GetAxisDeltaRotation(float InAxisRotationRate, float DeltaTime) const
{
